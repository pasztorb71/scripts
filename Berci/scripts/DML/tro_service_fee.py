...
if __name__ == '__main__':
    ...
    postfix = '_1'
    with open('c:/Users/bertalan.pasztor/Downloads/tro_service_fee_init_0.1.csv', 'r', encoding='utf8') as f:
        ...
        for line in f.readlines()[0:]:
            ...
            print(f"('{str(xid) + postfix}', '0', '{arr[0]}' ,'{arr[1].replace(' ','').replace(',','.')}', "
                  f"{arr[2].replace(' ','').replace(',','.')}, {arr[3].replace(',','.')}, to_date('{arr[4]}','DD.MM.YYYY'), '{arr[5]}', '{arr[6].strip()}'),")
            xid += 1