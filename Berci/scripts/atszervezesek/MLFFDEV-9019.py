...
def sema_atszervezes(repos):
    for repo in repos[1:]:
        ...
        utils.copy_file(base+src+'/liquibase/core_customer/_all-modules/schema/refresh_table_roles.sql',
                        base+repo+'/liquibase/'+db_path+'/_all-modules/schema/refresh_table_roles.sql')
        utils.copy_file(base+src+'/liquibase/core_customer/_all-modules/schema/procedures/add_privileges_to_all_tables.sql',
                        base+repo+'/liquibase/'+db_path+'/_all-modules/schema/procedures/add_privileges_to_all_tables.sql')
        utils.copy_file(base+src+'/liquibase/core_customer/_all-modules/schema/procedures/add_privileges_to_table.sql',
                        base+repo+'/liquibase/'+db_path+'/_all-modules/schema/procedures/add_privileges_to_table.sql')
        utils.append_to_file_after_line_last_occurence(base+repo+'/liquibase/'+db_path+'/'+schema+'/schema-versions.xml',
    ...
    <!-- =================================================================================== -->""")
        continue
        utils.copy_file_and_replace(base+src+'/run.sh', base+repo+'/run.sh', to_replace)
        ...
        utils.copy_file_and_replace(base+src+'/README.adoc', base+repo+'/README.adoc', to_replace)
