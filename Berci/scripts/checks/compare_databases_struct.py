...
def get_databases_from_envs(env1: str, env2: str, reload=False):
    if reload == True:
        dblist1 = Environment.Env(env1).databases
        dblist2 = Environment.Env(env2).databases
        with open(f'../backup/{env1}_databases.yml', 'w') as f:
            f.writelines([str(db)+'\n' for db in dblist1])
        with open(f'../backup/{env2}_databases.yml', 'w') as f:
            f.writelines([str(db)+'\n' for db in dblist2])
    else:
        with open(f'../backup/{env1}_databases.yml') as f:
            dblist1 = []
            for db in f.read().split('\n')[:-1]:
                port, name = db.split('__')
                dblist1.append(Database(name, port))
        with open(f'../backup/{env2}_databases.yml') as f:
            dblist2 = []
            for db in f.read().split('\n')[:-1]:
                port, name = db.split('__')
                dblist2.append(Database(name, port))
    return dblist1, dblist2

if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(asctime)s - %(message)s")
    dblist1, dblist2 = get_databases_from_envs('sandbox', 'prod', reload=False)
